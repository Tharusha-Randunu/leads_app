...
class LeadsProcessor:
    ...
    def standardize_phone_number(self, phone_str):
        ...
        if isinstance(phone_str, pd.Series):
            return phone_str.apply(self.standardize_phone_number)
    ...
    def load_files_from_folder(self, folder_path):
        ...
        try:
                ...
                if col_type in ['Name', 'Phone', 'Email', 'City']:
                    ...
                    if columns:
                        ...
                        for idx in range(len(self.df)):
                            value = ''
                            for col in columns:
                                if pd.notna(self.df.iloc[idx][col]) and str(self.df.iloc[idx][col]).strip() != '':
                                    value = str(self.df.iloc[idx][col]).strip()
                                    break
                    ...
                    for col in columns:
                        standard_columns[col] = self.df[col]
            ...
            print(f"Columns after standardization: {list(self.df.columns)}")
            ...
            update_columns = [col for col in self.df.columns if self.identify_column_type(col) == 'Update']
            if update_columns:
                ...
                self.df['Updates'] = self.df.apply(merge_updates, axis=1)
            ...
            other_columns = [col for col in self.df.columns if col not in standard_order]
